...
def prepare_throughput_forecast_table():
    ...
    with engine.begin() as conn:
        ...
        columns = {
            row._mapping["Field"]: row._mapping["Type"].lower()
            for row in conn.execute(text("SHOW COLUMNS FROM `throughput_forecast`")).fetchall()
        }
        timestamp_type = columns.get("timestamp", "")
        if "datetime" not in timestamp_type and "timestamp" not in timestamp_type:
            conn.execute(text("ALTER TABLE `throughput_forecast` ADD COLUMN `timestamp_dt` DATETIME"))
            rows = conn.execute(text("SELECT `id`, `timestamp` FROM `throughput_forecast`")).fetchall()
            for row in rows:
                try:
                    ts = _normalize_time_value(row._mapping["timestamp"])
                except ValueError:
                    ts = pd.Timestamp("1900-01-01")
                conn.execute(
                    text("UPDATE `throughput_forecast` SET `timestamp_dt` = :ts WHERE `id` = :id"),
                    {"ts": ts, "id": row._mapping["id"]},
                )
            conn.execute(text("ALTER TABLE `throughput_forecast` DROP COLUMN `timestamp`"))
            conn.execute(text("ALTER TABLE `throughput_forecast` CHANGE COLUMN `timestamp_dt` `timestamp` DATETIME NOT NULL"))
