...
class VietnamRelay:
    ...
    def _doh_resolve(self, host: str) -> str | None:
        ...
        try:
            ...
            pass
        ip = ip or GOV_IP_FALLBACK.get(host)
        if ip:
...
def preflight_check(relay: VietnamRelay, pf_cfg: dict) -> None:
    ...
    try:
        ...
        return
    except Exception as first_error:  # noqa: BLE001 目标不通，进一步分级
        status, detail = relay.probe()
    ...
    hints = {
        ...
        "ok": (
            "越南代理握手正常，但测试页 {url} 无响应——可能是 nso.gov.vn 临时故障，稍后重试。"
        ),
    }
    msg = hints.get(status, "越南代理不可用").format(
        host=relay.host, port=relay.port, url=test_url)
    raise VietnamProxyError(f"[越南代理预检失败/{status}] {msg}\n  底层明细: {detail}")
